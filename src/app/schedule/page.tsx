    ...
    <div className="space-y-6">
      ...
      {isOwner(me) && (
        <section>
          ...
          <Card className="p-4">
            {ownerState === "break" || ownerState === "lunch" || ownerState === "meeting" || ownerState === "appointment" || ownerState === "errand" || ownerState === "training" || ownerState === "bathroom" ? (
              <div className="flex flex-wrap items-center gap-3">
                <span className={`rounded-full px-3 py-1 text-sm font-bold ${ownerState === "meeting" ? "bg-violet-100 text-violet-800" : ownerState === "appointment" ? "bg-indigo-100 text-indigo-800" : ownerState === "errand" ? "bg-sky-100 text-sky-800" : ownerState === "training" ? "bg-teal-100 text-teal-800" : ownerState === "bathroom" ? "bg-stone-100 text-stone-700" : ownerState === "lunch" ? "bg-orange-100 text-orange-800" : "bg-amber-100 text-amber-800"}`}>
                  {ownerState === "meeting" ? "👥 In a meeting" : ownerState === "appointment" ? "🗓️ At an appointment" : ownerState === "errand" ? "🚗 Running an errand" : ownerState === "training" ? "🎓 In training" : ownerState === "bathroom" ? "💩 Bathroom break" : ownerState === "lunch" ? "🍔 On lunch" : "☕ On break"}
                </span>
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <form action={punch}><input type="hidden" name="kind" value="bathroom_start" /><button className="rounded-lg bg-stone-200 px-4 py-2 text-sm font-semibold text-stone-700 hover:bg-stone-300">💩 Bathroom</button></form>
              </div>
            )}
        ...
        <section>
          ...
          <Card className="divide-y divide-slate-100 p-0">
            {users.filter((u) => !isOwner(u)).map((u) => {
              ...
              const present = st === "online" || st === "break" || st === "lunch" || st === "meeting" || st === "appointment" || st === "errand" || st === "training" || st === "bathroom";
