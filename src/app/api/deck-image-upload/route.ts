...
/** Signed URL so the browser can upload a slide image straight to Supabase Storage. */
export async function POST(request: Request) {
  const me = await getCurrentUser();
  if (!me || !isManager(me)) return NextResponse.json({ error: "unauthorized" }, { status: 401 });
  ...
  const path = `slides/${new Date().toISOString().slice(0, 10)}/${randomUUID()}.${ext}`;
