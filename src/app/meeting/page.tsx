          ...
          <Card className="mt-3 p-5">
            ...
            <div className="space-y-2">
              {deckSlides.map((slide, i) => (
                <div key={slide.id} className={`rounded-lg border ${slide.active ? "border-slate-200" : "border-slate-100 bg-slate-50 opacity-60"}`}>
                  {/* Row: thumbnail + label + move / hide / delete (always visible) */}
                  ...
                  {/* Edit fields — always shown, pre-filled with this slide's current content */}
                  <form action={editDeckSlide} className="space-y-2 border-t border-slate-200 bg-slate-50/60 p-3">
                    <input type="hidden" name="id" value={slide.id} />
                    <div className="text-[11px] font-bold uppercase tracking-wide text-slate-400">Edit this slide</div>
                    <div className="flex gap-4 text-sm text-slate-600">
                    ...
                    <textarea name="body" rows={2} defaultValue={slide.body} placeholder="Body text (shows on the slide)" className={inputCls} />
